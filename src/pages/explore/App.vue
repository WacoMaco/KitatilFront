...
export default {
    ...
    if (this.$cookies.get("user_type") == "com") {
      if (window.location.search) {
        if (window.location.search.split("?")[1].split("&")) {
          ...
          var url_guarda_pagos = `https://api5-datame.herokuapp.com/api/v1/pagos/accept_paypal_payment/${paymentId}/${token_paypal}/${payerID}/`;
          this.$http
  ...
  methods: {
      ...
      this.$bvModal.msgBoxConfirm(text,  {
        ...
        if(value === true){
          ...
          if (
            ...
            this.$http
              .post("https://api5-datame.herokuapp.com/api/v1/apply", formApply, {
                headers: { Authorization: token }
              })
              .then(result => {
    ...
    getCompany: function(companyId) {
      var token = "JWT " + this.$cookies.get("token");
      this.$http
        .get("https://api5-datame.herokuapp.com/api/v1/company?companyId=" + companyId, {
          headers: { Authorization: token }
        })
        .then(result => {
    ...
    createOffer() {
      ...
      if (this.messages.length > 0) {
        ...
        this.$http
          .post("https://api5-datame.herokuapp.com/api/v1/offer", formData, {
            headers: { Authorization: token }
          })
          .then(result => {
            // alert(result.data.message)
            ...
            this.$http
              .get(
                `https://api5-datame.herokuapp.com/api/v1/pagos/create_papyal_payment/${offer_created}/`,
                { headers: { Authorization: token } }
              )
              .then(result => {
      ...
      var token = "JWT " + this.$cookies.get("token");
       this.$bvModal.msgBoxConfirm(text,  {
           ...
           cancelTitle:this.$t('cancel')
            ...
            if(value === true){
              this.$http
                .delete("https://api5-datame.herokuapp.com/api/v1/company/offer/" + id, {
                  headers: {
                    Authorization: token
                  }
                })
    ...
    onSubmit() {
      ...
      if (
        ...
        this.$http
          .get(
            `https://api5-datame.herokuapp.com/api/v1/offer?search_title=${
    ...
    updateOffer() {
      ...
      this.$http
        .post(
          "https://api5-datame.herokuapp.com/api/v2/change_offer/" + this.offerId,
          formData,
          { headers: { Authorization: token } }
        )
        .then(result => {
